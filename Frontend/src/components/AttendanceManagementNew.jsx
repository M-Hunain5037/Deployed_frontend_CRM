            ...
            <div className="p-4 bg-white rounded-xl border border-slate-100 shadow-sm">
              <div className="text-xs font-semibold text-slate-500 mb-1">Working Time</div>
                <div className="text-lg font-bold text-slate-800">
                  {(() => {
                    // If employee has checked out, show recorded working time
                    if (employee.check_out_time) return formatMinutesShort(employee.net_working_time_minutes || 0);
                    ...
                    if (employee.check_in_time && employee.attendance_date) {
                      try {
                        const [y, m, d] = employee.attendance_date.split('-').map(Number);
                        const [hh, mm, ss] = (employee.check_in_time || '00:00:00').split(':').map(Number);
                        const checkInDate = new Date(y, (m || 1) - 1, d || 1, hh || 0, mm || 0, ss || 0);
                        const diffMins = Math.max(0, Math.floor((nowTime - checkInDate.getTime()) / 60000));
                        return `${formatMinutesShort(diffMins)} (ongoing)`;
                      } catch (e) {
