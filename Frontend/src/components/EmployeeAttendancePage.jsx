  ...
  useEffect(() => {
    const fetchMonthlyData = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/api/v1/attendance/monthly/${employeeId}?year=${selectedYear}&month=${apiMonth}`,
          {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
...
export function EmployeeAttendancePage() {
  ...
  const fetchAndRestoreOngoingBreaks = async (breakRulesData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/v1/attendance/ongoing-breaks/${employeeId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const fetchTodayBreaksFromDB = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/v1/attendance/today-breaks/${employeeId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const saveBreakStart = async (breakType, startTime) => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/v1/attendance/break-start', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const autoSaveBreakProgress = async (breakType, startTime) => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/v1/attendance/break-progress', {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleBreakEnd = (breakType) => {
    ...
    const updateBreakEnd = async () => {
      try {
        ...
        const response = await fetch('http://localhost:5000/api/v1/attendance/break-end', {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-purple-600">
                        {(breakTypes.reduce((total, breakType) => total + (breakData[breakType.id]?.breakCount || 0), 0)) + todayBreaksFromDB.length}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-purple-600">
                        {formatDuration(
                          // Both breakSummary.totalDuration and DB break durations are in minutes
                          (breakSummary.totalDuration || 0) +
                  ...
                  <div className="mt-3 pt-3 border-t border-purple-200">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      {breakTypes.map((breakType) => {
                        ...
                        const totalCount = (breakData[breakType.id]?.breakCount || 0) + dbBreaks.length;
